...
tk.Button(master, 
          text='Jouer', 
          command=lambda: quitPopup()).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
...
def main_game():
	...
	while not game_over:
		...
		if turn == AI and not game_over:				
			...
			if is_valid_location(board, col):
				...
				if winning_move(board, AI_PIECE):
					label = myfont.render("l'IA a gagné", 1, YELLOW)
					screen.blit(label, (40,10))
					global score_AI
					score_AI += 1
					game_over = True
	...
	message = tk.Label(master, text="Score " + name1 + " : " + str(score_player))
	message.pack()
	message = tk.Label(master, text="Score " + 'AI' + " : " + str(score_AI))
	message.pack()
